...
// Session service specific configurations
const authServiceClient = createServiceClient('auth', process.env.AUTH_SERVICE_URL || 'http://auth-service:3000', {
    logger,
    headers: {
        'X-Service-Name': 'session-service',
        'Content-Type': 'application/json'
    }
});

const reportServiceClient = createServiceClient('report', process.env.REPORT_SERVICE_URL || 'http://report-service:3005', {
    logger,
    headers: {
        'X-Service-Name': 'session-service',
        'Content-Type': 'application/json'
    ...
    async execute(operation, options = {}) {
        ...
        for (let i = 0; i <= maxRetries; i++) {
            try {
                ...
                lastError = error;
                logger.warn(`MongoDB 작업 실패, 재시도 중... (${i + 1}/${maxRetries + 1})`, { 
                    error: error.message,
                    operation: options.operationName || 'mongodb_operation'
                });
...
// Register fallbacks
authServiceClient.registerFallback('validateToken', async (error) => {
    logger.warn('Auth service fallback triggered for token validation', { error: error.message });
    
    // For security, we should fail closed rather than open
    throw new Error('Authentication service unavailable');
});

reportServiceClient.registerFallback('createReport', async (error) => {
    logger.warn('Report service fallback triggered', { error: error.message });
    
    // Queue the report generation request
    try {
        const { redisClient } = require('../config/redis');
        await redisClient.rPush('report:queue', JSON.stringify({
        ...
        return {
            status: 'queued',
            message: 'Report generation request queued',
            _isFallback: true
        };
    } catch (queueError) {
        logger.error('Failed to queue report generation', { error: queueError.message });
        throw queueError;
    }
});

// Database resilience wrapper
async function withMongoResilience(operation, options = {}) {
    return mongoResilience.execute(operation, {
        operationName: options.operationName || 'mongodb_operation',
        ...options
    });
}
